...
export async function sendResetPasswordEmail(email, token) {
  ...
  await sendEmail({
    to: email,
    from: "Atmin Edunnes <noreply@gmail.com>",
...
export async function sendOtpEmail(email, otp) {
  await sendEmail({
    to: email,
    from: "Atmin Edunnes<3 <noreply@gmail.com>",
    subject: "OTP Verification",
    html: `
    <h1>OTP Verification</h1>
    <h2>Thank you for choosing Edunnes! To ensure the security of your account, we require you to verify your identity using the following One-Time Password (OTP)</p>
    <h3>Your OTP: ${otp}</h3>
