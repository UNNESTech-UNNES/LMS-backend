...
import * as Models from "../models/course_material.js";
...
export async function getCourseMaterialByCourseId(courseId) {
  ...
  const courseMaterialsIds = courseMaterials.map(({ dataValues: { id } }) => id);

  return courseMaterialsIds;
}

/** @param {string[]} materialId */
export function destroyMaterial(materialId) {
  return CourseMaterial.destroy({
    where: { id: { [Op.in]: materialId } },
  });
}
